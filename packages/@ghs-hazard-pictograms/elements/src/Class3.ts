...
const _Attrs = `xmlns="http://www.w3.org/2000/svg" viewBox="0 0 103.5 103.425"`;
...
const _DefaultHeight = `103.425`;
const _h = (s: string) => s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/'/g, '&#39;').replace(/"/g, '&quot;');
let _instances = 0;

export class Class3 extends HTMLElement {
  static readonly tagName = 'ghs-class-3';
  static readonly observedAttributes = ['title', 'description', 'width', 'height', 'aria-label'];

  private readonly _uid = `ghs-class-3-${++_instances}`;
