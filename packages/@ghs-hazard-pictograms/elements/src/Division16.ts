...
const _Attrs = `xmlns="http://www.w3.org/2000/svg" viewBox="0 0 103.125 103.125"`;
...
const _DefaultWidth = `103.125`;
const _DefaultHeight = `103.125`;
const _h = (s: string) => s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/'/g, '&#39;').replace(/"/g, '&quot;');
let _instances = 0;

export class Division16 extends HTMLElement {
  static readonly tagName = 'ghs-division-1-6';
  static readonly observedAttributes = ['title', 'description', 'width', 'height', 'aria-label'];

  private readonly _uid = `ghs-division-1-6-${++_instances}`;
