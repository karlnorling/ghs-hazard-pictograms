...
/**
 * Derives a short CSS class name from an SVG asset path.
 *
 * - GHS core pictograms: `ghs-ghsNN` (e.g. `ghs-ghs01`) — from the parent directory
 *   name which begins with the GHS code.
 * - Transport pictograms: `ghs-{slug}` (e.g. `ghs-adr-1-4`, `ghs-un-8`) — from the
 *   SVG filename with common verbose prefixes stripped.
 */
const shortCssClassName = (key: string): string => {
...
const createCSSSprite = async (): Promise<void> => {
  ...
  const svgFiles = globSync(path.join(ASSETS_ROOT, '**', '*.svg'))
    ...
    .filter((f) => !/ /.test(f)) // skip legacy folders that contain spaces (duplicates)
    .filter((f) => !/_\d+x\d+\.svg$/.test(f)); // skip pre-sized variants
  ...
  for (const file of svgFiles) {
    const key = path.relative(ASSETS_ROOT, file).replace(/\\/g, '/');
    const className = shortCssClassName(key);
